...
import com.slaviboy.colorpicker.window.SaturationLightnessSquare

/**
 * Full HSL color picker (the original library's `RectangularHSL`): a Saturation/Lightness
 * square, a Hue slider, an Alpha slider, and an optional value text field, laid out in a row.
 */
@Composable
fun RectangularHslColorPicker(
    ...
    Column(modifier) {
        Row(horizontalArrangement = Arrangement.spacedBy(spacing)) {
            SaturationLightnessSquare(state, Modifier.size(mainSize), style)
...
@Composable
private fun RectangularHslColorPickerPreview() {
    RectangularHslColorPicker(state = rememberColorPickerState(Color(0xFF3F8CB5)))
}
